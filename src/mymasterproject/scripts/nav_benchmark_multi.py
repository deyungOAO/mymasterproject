...
class NavBenchmarkMulti(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('output_csv',         '/home/deyung/ros2_newws/docs/nav_benchmark_results.csv')
    ...
    def _after_reset(self):
        ...
        # Clear stale path one more time now that robot has settled
        empty_path = Path()
        empty_path.header.stamp    = self.get_clock().now().to_msg()
        empty_path.header.frame_id = self.goal_frame
    ...
    def _reset_robot(self):
        ...
        # Stop the robot and clear the existing path so DWA/planner start fresh
        self.cmd_pub.publish(Twist())
        empty_path = Path()
        empty_path.header.stamp    = self.get_clock().now().to_msg()
        empty_path.header.frame_id = self.goal_frame
        self.path_pub.publish(empty_path)
        self.get_logger().info("  ✓ Stop + path cleared")
        try:
            cmd = [
                'gz', 'service',
    ...
    def _watchdog(self):
        ...
        if stuck_for >= self.STUCK_TIME_S and elapsed > 30.0:
